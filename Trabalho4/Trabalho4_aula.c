...
int **fourSum( int *nums, int numsSize, int target,
	...
	for ( int i = 0; i < numsSize - 3; i++ ) {
		...
		while ( pont2 < pont3 ) {
			...
			pont4 = numsSize - 1;
			while ( pont3 < pont4 ) {
				long long int test = nums[pont1] + nums[pont2] + nums[pont3] + nums[pont4];
				...
				if ( test < target ) {
					...
					while ( pont3 < pont4 && nums[pont3] == nums[pont4 - 1] ) {
						pont4--;
					}
					pont4--;
				} else if ( test == target ) {
			...
			while ( pont2 < pont3 && nums[pont2] == nums[pont2 + 1] ) {
				pont2++;
			}
			pont2++;
		}

		while ( pont1 < pont2 && nums[pont1] == nums[pont1 + 1] ) {
			pont1++;
			i++;
		}
	}
